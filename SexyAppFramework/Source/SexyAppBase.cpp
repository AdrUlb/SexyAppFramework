...
std::string SexyAppBase::GetProductVersion(const std::string& thePath)
{
	...
	if (aSize > 0)
	{
		...
		char aSubBlock1[] = R"(\StringFileInfo\040904B0\ProductVersion)";
		char aSubBlock2[] = R"(\StringFileInfo\040904E4\ProductVersion)";
		char* aBuffer;
...
bool SexyAppBase::RegistryWrite(const std::string& theValueName, ulong theType, const uchar* theValue, ulong theLength)
{
	...
	if (aResult != ERROR_SUCCESS)
	{
		ulong aDisp;
		char aClass[] = "Key";
		aResult = RegCreateKeyExA(HKEY_CURRENT_USER, aKeyName.c_str(), 0, aClass, REG_OPTION_NON_VOLATILE,
...
//  	if (CheckForVista())
	...
	if (mSoundManager == NULL)
		mSoundManager = new DSoundManager(mNoSoundNeeded?NULL:mHWnd);
